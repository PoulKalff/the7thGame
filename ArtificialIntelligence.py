...
def dumpBoard(board, highlight=None):
	"""	Helper function to visualize squares
		   [highlight] = [(x, y), (x, y)] :	Two fields on the board to highlight 
	"""
	if highlight:
		print(f'\n   Highlighting move: ({highlight[0][0]},{highlight[0][1]} ---> {highlight[1][0]},{highlight[1][1]})')
	print('   *---------------*')
	for y in range(7):
		print('   | ', end='')
		for x in range(7):
			if highlight and highlight[0][0] == x and highlight[0][1] == y:
				print(f"{TerminalColors.GREEN}" + str(board[x][y]) + f"{TerminalColors.RESET} ", end='')
			elif highlight and highlight[1][0] == x and highlight[1][1] == y:
				print(f"{TerminalColors.RED}" + str(board[x][y]) + f"{TerminalColors.RESET} ", end='')
			else:
...
class AI:
	...
	def analyzeMove(self, move):
		...
		for cm in counterMoves:
			adjacentCm, tmp = self._getNearSquares(cm.xTo, cm.yTo)		# getting squares surrounding the TO-field 
			piecesTaken = 0 
